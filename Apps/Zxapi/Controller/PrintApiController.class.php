...
class PrintApiController extends Controller {
    ...
    public function index(){
    	$msg =array('state'=>'0','msg'=>"参数有误！");
		echo json_encode($msg);
    }
 	/*
 	  //订单打印
	  @param expPlatform 0快递鸟 1快递100
	  @param expCompany 快递公司
 	 */
    public function order_print(){
  		$QY_FWDEKEY = C('QY_FWDEKEY');
    	$od_id = intval(trim(I('post.od_id', 0)));
    	$expPF = intval(trim(I('post.expPF', 0)));
    	$expID =trim(I('post.expID',''));
    	$expNcode =trim(I('post.expNcode',''));
    	$ttamp = intval(trim(I('post.ttamp', 0)));
    	$ttoken = trim(I('post.ttoken',''));
		if ($ttoken==MD5(MD5($QY_FWDEKEY).MD5($ttamp))&&$od_id>0&&$expNcode!='')
		{
            //
            $Eexpress=M('Express');
            $expOjb=$Eexpress->where('exp_id='.$expID)->find();
            if ($expOjb){
    			//获取构造的打印订单参数
    			$eorder=$this->orderPrintBuild($od_id,$expOjb);
    			//调用电子面单
    			$jsonParam = json_encode($eorder,JSON_UNESCAPED_UNICODE);
    			$jsonResult =json_decode($this->submitEOrder($jsonParam),true);
                if ($jsonResult&&$jsonResult['Success'])
                {

                    $ret=array('state' =>1,'result'=>$jsonResult,'msg'=>'');
                    echo json_encode($ret);
                    exit;
                }
                else
                {
                    $msg=$jsonResult['Reason'];
                    goto gotoEND;
                    exit;
                }
            }else
    ...
    /*
      ...
      @param expCompany 快递公司
     */
    public function order_printls(){
        $QY_FWDEKEY = C('QY_FWDEKEY');
        $pramArray =I('post.pramArray',[]); //批量订单订单id参数数组
        $expPF = intval(trim(I('post.expPF', 0)));
        $expID =trim(I('post.expID',''));
        $expNcode =trim(I('post.expNcode',''));
        $ttamp = intval(trim(I('post.ttamp', 0)));
        ...
        if ($ttoken==MD5(MD5($QY_FWDEKEY).MD5($ttamp))&&is_not_null($pramArray)&&$expNcode!='')
        {
            $printLsArray=[];
            $printLsErrArray=[];
            foreach ($pramArray as $k => $v){    
               //获取构造的打印订单参数
                $eorder=$this->orderPrintBuild($v['od_id'],$expNcode);
                //调用电子面单
                $jsonParam = json_encode($eorder,JSON_UNESCAPED_UNICODE);
                $jsonResult =json_decode($this->submitEOrder($jsonParam),true);
                if ($jsonResult&&$jsonResult['Success'])
                {
                     $printObj=(object)array();
                     $printObj['OrderCode']=$v['od_orderid'];
                     $printObj['PortName']=$v['od_protName'];
                     array_push($printLsArray,$printObj);
                }else
                {
                     $printErrObj=(object)array();
                     $printErrObj['OrderCode']=$v['od_orderid'];
                     array_push($printLsErrArray,$printObj);
                } 
            }

            //调用电子面单批量打印
            $jsonParam = json_encode($printLsArray,JSON_UNESCAPED_UNICODE);
            $jsonResult =json_decode($this->submitEOrderLS($jsonParam),true);
            if ($jsonResult&&$jsonResult['Success'])
            {

                $ret=array('state' =>1,'result'=>$jsonResult,'msg'=>'','odErrArray'=>$printLsErrArray);
                echo json_encode($ret);
                exit;
            }
            else
	 ...
	 *构造订单打印参数
	 *@param od_id 订单ID
	 *@param expNcode 快递公司编号ID
	*/
	public function orderPrintBuild($od_id,$expObj)
	{
		//构造电子面单提交信息
		$eorder=[];
		if ($od_id>0)
		{
			$Orders=M('Orders');
			$odOjb=$Orders->where('od_id='.$od_id)->field("od_id,od_orderid,od_oddlid,od_rcdlid,od_contact,od_tel,od_sheng,od_shi,od_qu,od_address")->find();
			if ($odOjb&&$expPF==0){
				// 获取省市区数据
				// $file =file_get_contents('./public/basha/js/AreaData_min.js'); //file_get_contents
	    		// $areaData=json_decode($file,true);
				// $dlArea=$this->areadate(31,3101,0);
				// var_dump($dlArea);
				// exit;
				//打印必填字段 
                $eorder["ShipperCode"] =$expObj['exp_niao_code'];
                if ($expObj['exp_username'])
                    $eorder["CustomerName"] =$expObj['exp_username'];
                if ($expObj['exp_password'])
				    $eorder["CustomerPwd"] =$expObj['exp_password'];
				$eorder["OrderCode"] =$odOjb['od_orderid'];
				$eorder["PayType"] = 1;
				$eorder["ExpType"] ="1";
				$sender = [];
				if ($odOjb['od_rcdlid']>0)
				{
					$Dealer=M('Dealer');
					$mapS=array();
					$mapS['dl_unitcode']=C('QY_UNITCODE');
					$mapS['dl_id']=$odOjb['od_rcdlid'];
					$mapS['dl_status']=1;
					$dlOjb=$Dealer->where($maps)->field('dl_id,dl_name,dl_tel,dl_sheng,dl_shi,dl_qu,dl_qustr,dl_address')->find();
					if ($dlOjb)
					{
						$dlArea=$this->areadate($dlOjb['dl_sheng'],$dlOjb['dl_shi'],$dlOjb['dl_qu']);
						$sender["Name"] =$dlOjb['dl_name'];
						$sender["Mobile"] =$dlOjb['dl_name'];
						$sender["ProvinceName"]=$dlArea['sheng'];
						$sender["CityName"] =$dlArea['shi'];
						$sender["ExpAreaName"] =$dlArea['qu'];
						$sender["Address"] =$dlOjb['dl_address'];
					}else
					{
						return [];
					}
				...
				{
					$sender["Name"] =C('QY_COMPANY');
					$sender["Mobile"] =C('QY_MOBILE');
					$sender["ProvinceName"]=C('QY_PRO');
					$sender["CityName"] =C('QY_CTITY');
					$sender["ExpAreaName"] =C('QY_AREA');
					$sender["Address"] =C('QY_ADDRESS');
				}
				
				if ($odOjb['od_oddlid']>0)
				{
					$rcdlArea=$this->areadate($odOjb['od_sheng'],$odOjb['od_shi'],$odOjb['od_qu']);
					$receiver = [];
					$receiver["Name"] =$odOjb['od_contact'];
					$receiver["Mobile"] =$odOjb['od_tel'];
					$receiver["ProvinceName"] =$rcdlArea['sheng'];
					$receiver["CityName"] =$rcdlArea['shi'];
					$receiver["ExpAreaName"] =$rcdlArea['qu'];
					$receiver["Address"] =$odOjb['od_address'];
				}else
				{
					return [];
				}	
				...
				$mapdt=array();
				$mapdt['oddt_unitcode']=C('QY_UNITCODE');
				$mapdt['oddt_odid']=$odOjb['od_id'];
				$mapdt['oddt_orderid']=$odOjb['od_orderid'];
				$mapdt['oddt_ectype']=0;
				$dtArray=$Orderdetail->where($mapdt)->field('oddt_proname,oddt_qty')->select();
				foreach ($dtArray as $k => $v) {
					$proName!=''?$proName=$proName.'|'.$v['oddt_proname']:$proName=$v['oddt_proname'];
					$proTotal+=$v['oddt_qty'];
				}
				if ($proName!='')
					$commodityOne["GoodsName"] =$proName;
				else
					$commodityOne["GoodsName"] = "其他";

                //产品数量
                if ($proTotal>0)
                $commodityOne["Goodsquantity"] =intval($proTotal,0); //备填
...
function submitEOrder($requestData){
    ...
    $datas['DataSign'] =$this->encrypt($requestData,C('KDN_APP_KEY'));
	$result=$this->sendPost(C('KDN_API_URL'),$datas);		
	//根据公司业务处理返回的信息......
	return $result;
}
 ...
 * Json方式 调用电子面单批量打印接口
 */
function submitEOrderLS($requestData){
    $datas = array(
        'EBusinessID' =>C('KDN_EBUSINESSID'),
        'RequestData' =>urlencode($requestData),
        'IsPriview' => '0' //是否预览 0不需要，1需要
    );
    $datas['DataSign'] =$this->encrypt($requestData,C('KDN_APP_KEY'));
    $result=$this->sendPost(C('KDN_API_URL'),$datas);       
    //根据公司业务处理返回的信息......
...
function sendPost($url, $datas){
    ...
    $httpheader.= "Content-Type:application/x-www-form-urlencoded;charset=utf8\r\n";
    $httpheader.= "Content-Length:" .strlen($post_data). "\r\n";
    $httpheader.= "Connection:close\r\n\r\n";
    $httpheader.= $post_data;
    $fd = fsockopen($url_info['host'], $url_info['port']);
    if(!$fd) {  
        return array('state' =>0,'msg' =>'参数有误！');  
    }  
    else {
...
//获取省市区
/***
* 传入$sheng,$shi,$qu(省市区的id)
* return array()
* array('sheng'=>省的名字, 
*       'shi'=>市的名字,
*       'qu'=>区的名字)
***/
public function areadate($sheng,$shi,$qu){
    $area_array=[];
    $sub_array=[];
    $area_array[0] = "请选择";
    $area_array[11]="北京市";
    ...
    if($sheng==0){
        ...
        $sheng_str=$area_array[$sheng];
    }
    if($shi==0||$sheng==0){
        $shi_str='';
    }else{
        $shi_str=$sub_array[$sheng][$shi];
    }
    if($qu==0||$shi==0||$sheng==0){
        $qu_str='';
    }else{
        $qu_str=$sub_arr[$shi][$qu];
    }
    
    $all_arr=array('sheng'=>$sheng_str,'shi'=>$shi_str,'qu'=>$qu_str);
    return $all_arr;
}
}