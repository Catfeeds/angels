...
//把防伪码转另一种显示 纯数字
function fwcode2fwcode($num){
    if(strlen($num)<=0){
        return false;
    }
    if(!preg_match("/^[0-9]{1,30}$/",$num)){
        return false;
    }
    $arry_sort=str_split($num,1);
    krsort($arry_sort);
    $num2='';
    foreach($arry_sort as $key =>$li){
           $num2.=9-$li;
    }
    return $num2;
}

//纯数字转换 需要key  $num和$key 位数要一样
function num2num($num,$key){
    if(strlen($num)<=0 || strlen($key)<=0 || strlen($num)!=strlen($key)){
        return false;
    }
    if(!preg_match("/^[0-9]{1,30}$/",$num)){
        return false;
    }
    if(!preg_match("/^[0-9]{1,30}$/",$key)){
        return false;
    }
    $tmp='';
    $num=strval($num);
...
function fwcode_to_result($fwcode='',$color=0,$uip='',$cn=1){
    ...
    if($color==1){
        if($codelen>=8){
            ...
            $code_b=substr($fwcode,4,$ai);//从第五位起截取$ai位  码2段
            $code_c=substr($fwcode,4+$ai,$bi);//截取$bi位       码3段
            $code_d=substr($fwcode,4+$ai+$bi,$ci);//截取$ci位   码4段
              
            $end_b=substr($code_b,-1,1);
            $ccode[1]=color_ch($code_b,$end_b);
               
            $end_c=substr($code_c,-1,1);
            $ccode[2]=color_ch($code_c,$end_c);
               
            $end_d=substr($code_d,-1,1);
            $ccode[3]=color_ch($code_d,$end_d);
               
            $color_ca=substr($end_b+$end_c+$end_d,-1,1);
            $ccode[0]=color_code($color_ca);
            //求和
			$li_b=substr($fwcode,4); 
			$arry_sort=str_split($li_b,1);
			$qh=0;
			foreach($arry_sort as $kk =>$vv){
				   $qh=$qh+$vv;
			}
			$ckey=substr($qh,-1,1);
			
			$ccode[0]=num2num(strval($ccode[0]),strval($ckey));
			$ccode[1]=num2num(strval($ccode[1]),strval($ckey));
			$ccode[2]=num2num(strval($ccode[2]),strval($ckey));
			$ccode[3]=num2num(strval($ccode[3]),strval($ckey));
							
            $color_str=$ccode[0].$ccode[1].$ccode[2].$ccode[3];
...
//返回颜色码
function color_ch($c='',$e=''){
       $cc=$e%2==0?substr($c,1,1):substr($c,0,1);
       return color_code($cc);
}
//色码转颜色
function code2color($clcode){
	$clstr='';
	switch ($clcode){
		case '1':
			$clstr='#000000';
 ...
 * @param  bool $ssl
 */
function http_request($method='get',$url,$data=array(),$ssl=true){
    //curl完成，先开启curl模块
    //初始化一个curl资源
    $curl = curl_init();
    //设置curl选项
