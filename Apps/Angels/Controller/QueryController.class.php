...
class QueryController extends CommController {
    ...
    public function index(){
        ...
        $from=intval(I('param.from',0));
        if ($from==1)
        $fwcode=base64_decode(urldecode($fwcode));
        $msg='';
        $is_checkcode=0;
		$ischuhuo=0; 
        if($fwcode!=''){
            if(!preg_match("/^[0-9]{10,27}$/",$fwcode)){
                $msg='您所查询的防伪码不存在，谨防假冒！';
                $fwcode='';
            }
        }
        if($fwcode!=''){
			...
			if($shdata){
                ...
                if($Proinfo){
                    ...
                    if(is_not_null($Proinfo['pro_pic2']) && file_exists($imgpath.$Proinfo['pro_pic2'])){
                        $prodata['pro_pic2']=$Proinfo['pro_pic2'];
                    }
			...
			$referer=substr($referer,0,300).'_Angels';
    ...
    public function ajaxres(){
        if (IS_POST) {
			...
			$msg=array('fwc'=>'','stat'=>'5','msg'=>'您所查询的防伪码不存在，谨防假冒！');
       ...
       if($referer==''){
		...
		if($qydata){
            ...
            $msg=array('fwc'=>$fwcode,'stat'=>'5','msg'=>'您所查询的防伪码不存在，谨防假冒！');
            echo json_encode($msg);
		    exit;
		}
        ...
        if($is_checkcode==1){
            ...
            if($checkcode==''){
                $msg=array('fwc'=>$fwcode,'stat'=>'9','msg'=>'您所查询的防伪码不存在，谨防假冒！');
                echo json_encode($msg);
                exit;
            }else{
