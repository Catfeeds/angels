...
class OrdersController extends CommController {
		...
		if($od_id>0 && $odbl_id>0){
			...
			if($data){
				...
				if(is_not_null($data['odbl_paypic']) && file_exists($imgpath2.$data['odbl_paypic'])){
					$data['odbl_paypic_str']='<a target="_blank" href="'.__ROOT__.'/Public/uploads/orders/'.$data['odbl_paypic'].'" ><img src="'.__ROOT__.'/Public/uploads/orders/'.$data['odbl_paypic'].'"   border="0"  style="width:20%;"  ></a>';
				}else{
				...
				if($data['odbl_state']==1 || $data['odbl_state']==2 || $data['odbl_state']==3 ){
					if($data['od_express']>0){
						$caozuostr='<div class="ui-btn ui-btn-primary"   data-href="'.U('./Demo82/Orders/odfinishship?od_id='.$data['od_id'].'&odbl_id='.$data['odbl_id'].'&od_state='.$od_state.'').'"  >修改物流</div> ';
					}else{
						$caozuostr='<div class="ui-btn ui-btn-primary"   data-href="'.U('./Demo82/Orders/odfinishship?od_id='.$data['od_id'].'&odbl_id='.$data['odbl_id'].'&od_state='.$od_state.'').'"  >完成发货</div> ';
					}
				}
				
				//取消订单
				if($data['odbl_state']==0 || $data['odbl_state']==1 ){
					$caozuostr.='<div class="ui-btn ui-btn-primary"   data-href="'.U('./Demo82/Orders/canceldlorder?state=9&od_id='.$data['od_id'].'&odbl_id='.$data['odbl_id'].'&od_state='.$od_state.'').'"  >取消订单</div> ';
				}
				...
				if($data['odbl_state']==0 || $data['odbl_state']==1){	 
					...
					if ($state==9)
					{
						...
						if($data['od_paymode']==1)
						{
							$Balance=M('Balance');
							$blmap=array();
							$blmap['bl_unitcode']=session('unitcode');
    ...
    public function cporders(){
        ...
        foreach($list as $k=>$v){
			...
			//允许操作
			$caozuostr='<a href="'.U('./Mp/Orders/cporderdetail?od_id='.$v['od_id'].'&odbl_id='.$v['odbl_id'].'').'"  >订单详细</a><br>';
			
           //确认订单
			if($v['odbl_state']==0){
				$caozuostr.='<a href="'.U('./Mp/Orders/cancelorder?state=1&od_id='.$v['od_id'].'&odbl_id='.$v['odbl_id'].'').'"  >确认订单</a><br>';
			}
			
			//取消订单
			if($v['od_state']==0 || $v['od_state']==1 ){
				$caozuostr.='<a href="'.U('./Mp/Orders/cancelorder?state=9&od_id='.$v['od_id'].'&odbl_id='.$v['odbl_id'].'').'"  >取消订单</a><br>';
			}
					...
					if($data['odbl_state']==1 || $data['odbl_state']==2 || $data['odbl_state']==3){
						if($oddt_totalqty>0){
							if($oddt_totalqty>$data3){
								$data2[$kk]['oddt_shipment']='<a class="abotton"  href="'.U('./Mp/Orders/odshipscan?od_id='.$data['od_id'].'&odbl_id='.$data['odbl_id'].'&oddt_id='.$vv['oddt_id'].'').'"   >出 货</a>';
							}else{
    ...
    public function odfinishship(){
		...
		$odbl_id=intval(I('param.odbl_id',0));
									...
									foreach($oddetail as $kk=>$vv){
										...
										if($data7){
											if($data7['pfl_fanli1']>0){
												$map8=array();
												$data8=array();
												$map8['yfk_unitcode'] = session('unitcode');
												$map8['yfk_type'] = 2;
												...
												if(!$data8){
													...
													$data5=array();
													$data5['yfk_unitcode'] = session('unitcode');
						...
						if($data7){
							...
							if($data7['pri_jifen']>0){
								...
								if(!$data8){
									...
									if(session('unitcode')=='2979'){ //旨来脂去 专用积分计算
   ...
   //公司处理所有订单 如：卡曼丽斯
	...
	//所有订单 可处理
    public function qborders(){
        $this->check_qypurview('13008',1);
        ...
        foreach($list as $k=>$v){
			...
			//允许操作
			$caozuostr='<a href="'.U('./Mp/Orders/qborderdetail?od_id='.$v['od_id'].'&odbl_id='.$v['odbl_id'].'').'"  >订单详细</a><br>';
			
           //确认订单
			if($v['odbl_state']==0){
				$caozuostr.='<a href="'.U('./Mp/Orders/qbcancelorder?state=1&od_id='.$v['od_id'].'&odbl_id='.$v['odbl_id'].'').'"  >确认订单</a><br>';
			}
			
			//取消订单
			if($v['od_state']==0 || $v['od_state']==1 ){
				$caozuostr.='<a href="'.U('./Mp/Orders/qbcancelorder?state=9&od_id='.$v['od_id'].'&odbl_id='.$v['odbl_id'].'').'"  >取消订单</a><br>';
			}
			
			//删除订单 只有已取消的才能删除
			if($v['od_state']==9 ){
			    $caozuostr.='<a   href="#"  onClick="javascript:var truthBeTold = window.confirm(\'该操作将彻底删除,谨慎操作!\'); if (truthBeTold) window.location.href=\''.U('Mp/Orders/deleteorder2?od_id='.$v['od_id']).'\';"  >删除</a> ';
			}
        ...
        $this->display('qborders');
    }
   
	//按订单出货扫码 
	public function qbodshipscan(){
        $this->check_qypurview('13008',1);
		...
		$this->assign('ordersbase', $order);
		$this->assign('ordersinfo', $data);
		$this->display('qbodshipscan');
	}
	
	
	//代理商按订单出货扫描结果
    public function qbodshipscanres(){
        $this->check_qypurview('13008',1);
		//--------------------------------
        ...
        $this->display('qbodshipscanres');
	}
	
	
	//代理商按订单出货扫描保存
    public function qbodshipscanres_save(){
        $this->check_qypurview('13008',1);
		//--------------------------------
        ...
        $this->display('qbodshipscanres_save');
	}
	
	
    //完成发货
    public function qbodfinishship(){
        $this->check_qypurview('13008',1);
		...
		$this->display('qbodfinishship');
    }
	
    //完成发货 保存
    public function qbodfinishship_save(){
        $this->check_qypurview('13008',1);
		...
		if($od_id>0 && $odbl_id>0){
			...
			if($data['od_express']<=0){
				...
				if($orderdealer){
					...
					$data2 = $Dltype->where($map2)->find();
					if($data2){
						$dlt_level=$data2['dlt_level'];  //下单代理级别
				
					}else{
						$this->error('代理级别不存在','',2);
				        exit;
					}
					
					$fanli_dlid1=$orderdealer['dl_id'];
					$fanli_dlname1=$orderdealer['dl_username'];
					$fanli_dlid1ok=1;
					
					//下单人的上家
					if($orderdealer['dl_belong']>0){
						
					
						$map4=array();
						$data4=array();
						$map4['dl_unitcode'] = session('unitcode');
						$map4['dl_id'] = $orderdealer['dl_belong'];  //上家代理
						
						$data4=$Dealer->where($map4)->find();
						if($data4){
							if($data4['dl_status']==1){
								$fanli_dlid2=$data4['dl_id'];
					            $fanli_dlname2=$data4['dl_username'];
								$fanli_dlid2ok=1;
							}else{
								$fanli_dlid2=$data4['dl_id'];
					            $fanli_dlname2=$data4['dl_username'];
							...
							//上家的上家
							if($data4['dl_belong']>0){

								$map5=array();
								$data5=array();
								$map5['dl_unitcode'] = session('unitcode');
								$map5['dl_id'] = $data4['dl_belong'];  //推荐人的推荐人

								$data5=$Dealer->where($map5)->find();
								if($data5){
									if($data5['dl_status']==1){
										$fanli_dlid3=$data5['dl_id'];
										$fanli_dlname3=$data5['dl_username'];
										$fanli_dlid3ok=1;
									}else{
										$fanli_dlid3=$data5['dl_id'];
										$fanli_dlname3=$data5['dl_username'];
					...
					if($fanli_dlid3>0){
						...
						if($fanli_dlid2>0 && $fanli_dlid2ok==1){
							$fanli_rate2=0.08;   //8%
						}
						
						if($fanli_dlid1>0 && $fanli_dlid1ok==1){
							$fanli_rate1=0.18;   //18%
						}
					}else{
						if($fanli_dlid2>0){
					...
					//写入返利
					if($fanli_dlid1>0 && $fanli_dlid1ok==1 && $fanli_rate1>0){
						$Fanlidetail = M('Fanlidetail');
						foreach($oddetail as $kk=>$vv){
							$map8=array();
							$data8=array();
							...
							if(!$data8){
								...
								$pfl_fanli1sum=$fanli_rate1*$vv['oddt_dlprice']*$vv['oddt_qty'];
							...
							//如果有设置2层返利
							if($fanli_dlid2>0 && $fanli_dlid2ok==1 && $fanli_rate2>0){
								$map8=array();
								$data8=array();
								$map8['fl_unitcode'] = session('unitcode');
								$map8['fl_type'] = 2;
								...
								$data8 = $Fanlidetail->where($map8)->find();
								if(!$data8){
									$pfl_fanli2sum=$fanli_rate2*$vv['oddt_dlprice']*$vv['oddt_qty'];
									...
									$data5['fl_level']  = 2;  //返利的层次，1-第一层返利 2-第二层返利
									$data5['fl_addtime']  = time();
									$data5['fl_remark'] ='代理 '.$fanli_dlname1.' 的下级代理 '.$orderdealer['dl_username'].' 订购 '.$vv['oddt_proname'].' 数量 '.$vv['oddt_qty'] ;
									$rs5=$Fanlidetail->create($data5,1);
									if($rs5){
										$Fanlidetail->add();
									}
							...
							//如果有设置3层返利
							if($fanli_dlid3>0 && $fanli_dlid3ok==1 && $fanli_rate3>0){
								$map8=array();
								$data8=array();
								$map8['fl_unitcode'] = session('unitcode');
								$map8['fl_type'] = 2;
								...
								$data8 = $Fanlidetail->where($map8)->find();
								if(!$data8){
									$pfl_fanli3sum=$fanli_rate3*$vv['oddt_dlprice']*$vv['oddt_qty'];
									...
									$data5['fl_level']  = 3;  //返利的层次，1-第一层返利 2-第二层返利
									$data5['fl_addtime']  = time();
									$data5['fl_remark'] ='代理 '.$fanli_dlname2.' 的下级代理 '.$fanli_dlname1.' 的下级代理 '.$orderdealer['dl_username'].' 订购 '.$vv['oddt_proname'].' 数量 '.$vv['oddt_qty'] ;
									$rs5=$Fanlidetail->create($data5,1);
									if($rs5){
										$Fanlidetail->add();
									}
			...
			$this->error('物流信息提交成功',U('Mp/Orders/qborderdetail/od_id/'.$od_id.'/odbl_id/'.$odbl_id.'/back/1'),2);
			exit;
		}else{
			$this->error('该订单记录不存在','',2);
			exit;
    ...
    public function qborderdetail(){
			...
			if($data){
				...
				foreach($data2 as $kk=>$vv){
					...
					if($data['odbl_state']==1 || $data['odbl_state']==2 || $data['odbl_state']==3){
						if($oddt_totalqty>0){
							if($oddt_totalqty>$data3){
								$data2[$kk]['oddt_shipment']='<a class="abotton"  href="'.U('./Mp/Orders/qbodshipscan?od_id='.$data['od_id'].'&odbl_id='.$data['odbl_id'].'&oddt_id='.$vv['oddt_id'].'').'"   >出 货</a>';
							}else{
								$data2[$kk]['oddt_shipment']='';
							}
						}else{
		...
		$this->display('qborderdetail');
    }
	
	
	//订单已发货记录
	public function qbodshiplist(){
		$this->check_qypurview('13008',1);
