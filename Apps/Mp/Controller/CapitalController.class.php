...
class CapitalController extends CommController {
   ...
   //          //余额 增加(有效的)
			...
			$list[$k]['yuesurplus']=$this->mybalance($v['dl_id']); //剩余余额
		}
    ...
    public function dlbalance(){
		...
		foreach($list as $k=>$v){
			...
			if($v['bl_receiveid']>0){
				...
				if ($v['bl_receiveid']==0)
				{
					$list[$k]['dl_name_str']='总公司';
					$list[$k]['dl_number']='总公司';
				}else
				{
					$list[$k]['dl_name_str']='-';
					$list[$k]['dl_number']='-';
		    ...
		    switch ($v['bl_type']) {
		    	...
		    	break;
		    	case '3':
		    		$list[$k]['bl_typestr']='返利提现款项增减';
		    	break;
		    	case '4':
		    		$list[$k]['bl_typestr']='充值款项';
		    	break;
				case '5':
		    		$list[$k]['bl_typestr']='余额提现';
		    	break;
		    	default:
		...
		if($pi_id>0){
			...
			if($data){
				...
				if($rs){
					...
					$blmap['bl_sendid']=$data['pi_receiveid'];
					$blmap['bl_receiveid']=$data['pi_dlid'];
					$blmap['bl_type']=4; //充值
					$blmap['bl_piid']=$pi_id; 
					$bldata=$Balance->where($blmap)->find();
					if ($bldata)
					{
						$blupdata=array();
						if ($pi_state==1)
						{
							$blupdata['bl_state']=1; //改为有效状态

						}
						else if ($pi_state==2)
						{
							$blupdata['bl_state']=0; //改为无效状态

						}
						$blupdata['bl_remark']=$pi_dealremark;
