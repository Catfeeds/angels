...
//礼品兑换
class JfexchangeController extends CommController {
	//礼品兑换列表
    public function index(){
        $this->check_qypurview('50002',1);
    ...
    public function deal_save(){
        ...
        if($map['exch_id']>0){
            ...
            $map['exch_unitcode']=session('unitcode');
            $Jfexchange= M('Jfexchange');
            $rs=$Jfexchange->where($map)->data($data)->save();
            ...
            if($rs){
                ...
                save_log($log_arr);
                //记录日志 end
                $this->success('处理成功',U('Mp/Jfexchange/view/exch_id/'.$map['exch_id']),1);
            }else{
               $this->error('处理失败','',1);
            }
