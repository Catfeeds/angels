    ...
    public function active(){
        ...
        if($data){
			...
			if($active==1 && $data['dl_startdate']<=0 ){
					...
					if($data['dl_sttype']>0){
						$map33=array();
						$map33['dlstt_unitcode']=session('unitcode');
						$map33['dlstt_id']=$data['dl_sttype'];
						$Dlsttype = M('Dlsttype');
						$data33 = $Dlsttype->where($map33)->find();
						if($data33){
							
						}else{
							$this->error('申请门店级别不存在','',2);
						}
					}else{
						$data33['dlstt_fanli1']=0;
						$data33['dlstt_fanli2']=0;
					}
				
                    //推荐返利 begin
					
					//如果级别2有推荐返利 如果有推荐人 并设置了返利 如：明臣眼科 公司发放
					if(session('unitcode')=='2891'){
						if($data['dl_referee']>0 && $data33['dlstt_fanli1']>0){
							$map4=array();
							...
							if($data4){
								...
								$data5['fl_money'] = $data33['dlstt_fanli1'];
								...
								$data5['fl_remark'] ='邀请 '.$data['dl_username'].' 成为 '.$data33['dlstt_name'];
								...
								if(!$data6){
									...
									if($rs5){
										...
										// if($rs5id){
										// 	//第一层返利成功后 检测是否有第二层返利
										// 	if($data4['dl_referee']>0 && $data33['dlstt_fanli2']>0){
										// 		$map6=array();
										// 		$map6['dl_id']=$data4['dl_referee'];
										...
										// 			$data5['fl_senddlid'] = 0; //发放返利的代理 0为公司发放
										// 			$data5['fl_type'] = 1; //返利分类 1-推荐返利 2-订单返利  11-提现减少返利 (1-10 增加返利 11-20 减少返利) 
										// 			$data5['fl_money'] = $data33['dlstt_fanli2'];
										// 			$data5['fl_refedlid'] = $data['dl_id']; //推荐返利中被推荐的代理
										// 			$data5['fl_oddlid'] = 0; //订单返利中 下单的代理
										// 			$data5['fl_odid'] = 0;  //订单返利中 订单流水id
										// 			$data5['fl_orderid']  = ''; //订单返利中 订单id
										...
										// 			$data5['fl_addtime']  = time();
										// 			$data5['fl_remark'] ='你邀请的 '.$data4['dl_username'].' 再邀请'.$data['dl_username'].' 成为 '.$data33['dlstt_name'];
										...
										// 			$map5['fl_type'] = 1;
										// 			$map5['fl_refedlid'] = $data['dl_id'];
										// 			$data7 = $Fanlidetail->where($map5)->find();
										// 			if(!$data7){
										// 				$rs5=$Fanlidetail->create($data5,1);
										// 				if($rs5){
										// 					$Fanlidetail->add();
					...
					if(session('unitcode')=='2971'){   //澳瀚斯返利 全由公司发放
						//如果有推荐人 并设置了返利
						if($data['dl_referee']>0 && $data3['dlt_fanli1']>0){
							...
							if($data4){
								...
								$Fanlidetail = M('Fanlidetail');
								//推荐人都有返利
								if($data['dl_level']>0 && $data4['dl_level']>0 ){
									$data5=array();
									$data5['fl_unitcode'] = session('unitcode');
									$data5['fl_dlid'] = $data4['dl_id']; //获得返利的代理
									$data5['fl_senddlid'] = 0; //发放返利的代理 0为公司发放
									...
									$data5['fl_level']  = 1;  //返利的层次，1-第一层返利 2-第二层返利
									$data5['fl_addtime']  = time();
									$data5['fl_remark'] ='邀请 '.$data['dl_username'].' 成为 '.$data3['dlt_name'];
									...
									if(!$data6){
										...
										if($rs5){
											...
											if($rs5id){
												//第一层返利成功后 检测是否有第二层返利
												if($data4['dl_referee']>0 && $data3['dlt_fanli2']>0){
													$map6=array();
													$map6['dl_id']=$data4['dl_referee'];
													$map6['dl_unitcode']=session('unitcode');
													$map6['dl_status']=1;
													...
													if($data6){
														//二层返利
														//如果推荐人的推荐人同级 则有返利
														if($data4['dl_level']==$data6['dl_level']){
															$data5=array();
															$data5['fl_unitcode'] = session('unitcode');
															$data5['fl_dlid'] = $data6['dl_id']; //获得返利的代理
															$data5['fl_senddlid'] = 0; //发放返利的代理 0为公司发放
															$data5['fl_type'] = 1; //返利分类 1-推荐返利 2-订单返利  11-提现减少返利 (1-10 增加返利 11-20 减少返利) 
															$data5['fl_money'] = $data3['dlt_fanli2'];
															$data5['fl_refedlid'] = $data['dl_id']; //推荐返利中被推荐的代理
															$data5['fl_oddlid'] = 0; //订单返利中 下单的代理
															$data5['fl_odid'] = 0;  //订单返利中 订单流水id
															$data5['fl_orderid']  = ''; //订单返利中 订单id
															...
															$data5['fl_level']  = 2;  //返利的层次，1-第一层返利 2-第二层返利
															$data5['fl_addtime']  = time();
															$data5['fl_remark'] ='你邀请的 '.$data4['dl_username'].' 再邀请'.$data['dl_username'].' 成为 '.$data3['dlt_name'];
						...
						//审核通过后 检测是否有推荐返利 级别1 【默认】推荐比自己高级的才有返利
						//如果有推荐人 并设置了返利
						if($data['dl_referee']>0 && $data3['dlt_fanli1']>0){
							$map4=array();
							$map4['dl_id']=$data['dl_referee'];
							...
							if($data4){
								//返利
								$Fanlidetail = M('Fanlidetail');
								//如果推荐人的级别低于申请人的级别 并且申请人的级别不是特约(6级) 则有返利
								if($data4['dl_level']>=$data['dl_level']&&$data['dl_level']<6){
									$data5=array();
									$data5['fl_unitcode'] = session('unitcode');
									$data5['fl_dlid'] = $data4['dl_id']; //获得返利的代理
