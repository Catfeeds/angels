...
//退换货管理
class ReturnableController extends CommController {
	
	//退换给公司的
    public function index(){
        $this->check_qypurview('13006',1);
        ...
        if($keyword!=''){
            ...
            $map['ret_barcode'] =array('LIKE', ''.$keyword.'%');
			
			$parameter['keyword']=$keyword;
        }

        $map['ret_unitcode']=session('unitcode');
		$map['ret_dealer']=0;   //ret_deliver--退换者   ret_dealer--退换收方

        $count = $Returnable->where($map)->count();
        $Page = new \Think\Page($count, C('PAGE_SIZE'),$parameter);
        $show = $Page->show();
        $list = $Returnable->where($map)->order('ret_id DESC')->limit(($Page->firstRow . ',') . $Page->listRows)->select();

        foreach($list as $k=>$v){
			//对应的产品
			$map2=array();
			$map2['pro_unitcode']=session('unitcode');
			$map2['pro_id'] = $v['ret_pro'];
			...
			if($v['ret_status']==0){
				$list[$k]['ret_statusstr']='新申请';
			}else if($v['ret_status']==1){
				$list[$k]['ret_statusstr']='同意退换';
			}else if($v['ret_status']==2){
				$list[$k]['ret_statusstr']='不同意';
			}else{
				$list[$k]['ret_statusstr']='未处理';
			}
			
			
			if($v['ret_type']==1){
				$list[$k]['ret_typestr']='换货';
			}else{
				$list[$k]['ret_typestr']='退货';
			}
			
			//退换代理
			if($v['ret_deliver']>0){
				$map2=array();
				$map2['dl_id']=$v['ret_deliver'];
				$map2['dl_unitcode']=session('unitcode');
				$data2=$Dealer->where($map2)->find();
				if($data2){
					$list[$k]['ret_deliver_name']=wxuserTextDecode2($data2['dl_name']).'('.$data2['dl_weixin'].')';
				}else{
					$list[$k]['ret_deliver_name']='';
				}
			}else{
				$list[$k]['ret_deliver_name']='';
			}
			
			$list[$k]['ret_dealerstr']='总公司';
