...
class GetcmresController extends Controller {
	//彩码接口 返回json格式
    public function index(){
        ...
        if($fwkey=='' || $fwcode=='' || $timestamp=='' || $signature==''){
            $msg=array('fwc'=>'','clr'=>'','stat'=>'5','msg'=>'请正确输入防伪码或扫二维码查询');
        ...
        if($deqycode=='zxFW315Com'){
            $qy_fwsecret=C('QY_FWSECRET');
            if($data){
                $qy_querytimes=$data['qy_querytimes'];  
                $is_localquery=0;
            }else{
                $qy_querytimes='1500';
            }
        ...
        if($qy_querytimes<$tlcount){
            ...
            if((time()-$querydate)<180){
                $msg=array('fwc'=>'','clr'=>'','stat'=>'5','msg'=>'对不起，因查询频繁，请稍后再试');
                echo json_encode($msg);
                exit;   
            }
        }
        ...
        if($is_localquery==1){
            ...
            if($data){
                ...
                $msg=array('fwc'=>'','clr'=>'','stat'=>'5','msg'=>'网络繁忙');
                echo json_encode($msg);
                exit;
            }

            $url="http://".$ip.":6080/cmq.asp?ip=".$uip."&fwcode=".$fwcode; 
            ...
            if($output===FALSE){
				...
				$data['tmp_clr']=$msg_arr['clr'];
                $data['tmp_ip']=$uip;
                $data['tmp_addtime']=time();
                $data['tmp_remark']=$msg_arr['msg'];
                $data['tmp_referer']=$referer.'_Local_APIGetcmres';
            ...
            $fwcoders=fwcode_to_result($fwcode,1,$uip,1);
            if($fwcoders===false){
                $msg_arr=array('fwc'=>$fwcode,'clr'=>'','stat'=>'5','msg'=>C('SEND_MESSAGE')['msg03']);
            }else{
                $msg_arr=$fwcoders;
            }
            ...
            $data['tmp_addtime']=time();
            $data['tmp_remark']=$msg_arr['msg'];
            $data['tmp_referer']=$referer.'_APIGetcmres';
            $Templist->create($data,1);
            $Templist->add();
