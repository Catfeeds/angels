...
<header class="ui-header ui-header-positive ui-border-b">
	<i class="ui-icon-return" data-href="<?php echo U('./Angels/Fanli/index');?>"  ></i><h1>提现记录</h1>
</header>
<section class="ui-container" >

<ul class="ui-list ui-list-text ui-list-link ui-border-tb">

<?php if(is_array($list)): foreach($list as $key=>$item): ?><li class="ui-border-t" data-href="<?php echo U('./Angels/Fanli/recashdetail?rc_id='.$item['rc_id'].'');?>" >
        <div class="ui-list-info">
			...
			<p class="ui-nowrap">向 <?php if(!empty($item["fl_sdl_name"])): echo ($item["fl_sdl_name"]); endif; ?> <?php if(!empty($item["fl_sdl_username"])): ?>(<?php echo ($item["fl_sdl_username"]); ?>)<?php endif; ?> <?php echo ($item["rc_str"]); ?></p>
        </div>
