...
<header class="ui-header ui-header-positive ui-border-b">
	<i class="ui-icon-return"  data-href="<?php echo U('./Angels/Dealer/index');?>"  ></i><h1>我的订单</h1>
</header>
<section class="ui-container" >
<div class="ui-tab" id="tab1">
    <ul class="ui-tab-nav ui-border-b">
        <li <?php if($od_state == '1' ): ?>class="current"<?php endif; ?>  data-href="<?php echo U('./Angels/Orders/index?od_state=1');?>" >待发货</li>
        <li <?php if($od_state == '3' ): ?>class="current"<?php endif; ?>  data-href="<?php echo U('./Angels/Orders/index?od_state=3');?>" >已发货</li>
        <li <?php if($od_state == '0' ): ?>class="current"<?php endif; ?>  data-href="<?php echo U('./Angels/Orders/index?od_state=0');?>" >待确认</li>
		<li <?php if($od_state == '8' ): ?>class="current"<?php endif; ?>  data-href="<?php echo U('./Angels/Orders/index?od_state=8');?>"  >已完成</li>
		<li <?php if($od_state == '9' ): ?>class="current"<?php endif; ?>  data-href="<?php echo U('./Angels/Orders/index?od_state=9');?>"  >已取消</li>
    </ul>
</div>
<?php if(is_array($orderlist)): foreach($orderlist as $key=>$item): ?><ul class="ui-list ui-list-text ui-border-tb">
	...
	<?php if(is_array($item['orderdetail'])): foreach($item['orderdetail'] as $key2=>$item2): ?><li class="ui-border-t">
        ...
        <div class="ui-list-info">
			...
			<p class="ui-nowrap">订购：<?php echo ($item2["oddt_qty"]); echo ($item2["oddt_prounits"]); echo ($item2["oddt_totalqty"]); ?>　已发：<?php echo ($item2["oddt_shipqty"]); ?> </p>
			
        </div>
    </li><?php endforeach; endif; ?>

	<li class="ui-border-t">
	    <h4 class="ui-nowrap"><?php echo ($item["od_contact"]); ?> (<?php echo ($item["od_tel"]); ?>)</h4>
		<div class="ui-txt-info"><?php echo (date('Y-m-d H:i:s',$item["od_addtime"])); ?></div>
	
	</li>
	<li class="ui-border-t">
	    <h4 class="ui-nowrap">总额：<?php echo (number_format($item["od_total"],2,'.','')); ?> 元</h4>
